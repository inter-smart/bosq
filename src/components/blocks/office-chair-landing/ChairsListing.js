...
export default function ChairsListing({ data, locale = "en" }) {
  ...
  const listDescription = isEn ? data?.listDescription : data?.listDescription_ar;
  const ctaLabel = isEn ? data?.cta?.label : data?.cta?.label_ar;
  const ctaHref = data?.cta?.href || "#";
  const label = isEn ? data?.cta?.label : data?.cta?.label_ar;
      ...
      <div className="container">
        ...
        <div className="flex flex-col lg:flex-row gap-7 lg:gap-12 xl:gap-7 2xl:gap-20">
          ...
          <div className="w-full lg:w-[425px] 2xl:w-[652px]">
            {description && (
              <div className={cn("typography sm:text-[13px] 2xl:text-[15px] 3xl:text-[18px] sm:[&_p,li]:text-[13px] 2xl:[&_p,li]:text-[15px] 3xl:[&_p,li]:text-[18px] [&_p,li]:leading-[1.8] lg:[&_li]:mb-2 2xl:[&_li]:mb-3 3xl:[&_li]:mb-5 2xl:[&_p]:mb-5 3xl:[&_p]:mb-8")}>
                {parse(description)}
              </div>
            )}
