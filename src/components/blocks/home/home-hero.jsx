...
export default function HomeHero({ data, locale, dropdownData, state }) {
  const t = useTranslations("home");
  const isEn = locale === "en";
  const [emblaReady, setEmblaReady] = useState(false);
  const [emblaRef, emblaApi] = useEmblaCarousel({ loop: false, direction: !isEn ? "rtl" : "ltr" }, [
      ...
      <div className="w-full max-w-full">
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex touch-pan-y touch-pinch-zoom">
            {parsedSlides?.map((item, index) => (
              ...
              >
                <div
                  className={cn("w-full h-full from-transparent to-black/40 absolute -z-1 inset-0 ", !isEn ? "bg-linear-to-r" : "bg-linear-to-l")}
                />
                ...
                ) : item?.media?.mobile?.path || item?.media?.mobile?.path_ar ? (
                  <>
                    <div className="absolute -z-2 inset-0 sm:hidden">
                      <Image
                        src={(isEn ? item?.media?.mobile?.path : item?.media?.mobile?.path_ar) || "/images/placeholder.png"}
                        alt={(isEn ? item?.media_alt : item?.media_alt_ar) ?? "slider image"}
                        fill
                        fetchPriority={index === 0 ? "high" : "auto"}
                        title={(isEn ? item?.media_alt : item?.media_alt_ar) ?? "slider image"}
                        sizes="100vw"
                    ...
                    <div className="absolute -z-2 inset-0 max-sm:hidden">
                      <Image
                        src={(isEn ? item?.media?.desktop?.path : item?.media?.desktop?.path_ar) || "/images/placeholder.png"}
                        alt={(isEn ? item?.media_alt : item?.media_alt_ar) ?? "slider image"}
                        title={(isEn ? item?.media_alt : item?.media_alt_ar) ?? "slider image"}
                        fill
                    ...
                    <div className="w-full xl:max-w-1/2 max-lg:px-4">
                      <Heading as="h1" size="heading1" className="line-clamp-4 leading-tight text-white mb-2 xl:mb-4 2xl:mb-6">
                        ...
                        <span
                          className={cn(
                            ...
                            !isEn ? "-translate-x-1 xl:-translate-x-2 " : "translate-x-1 xl:translate-x-2 ",
                          )}
                        />
                      </Heading>
                      ...
                      <div className="flex items-center gap-3 xl:gap-4">
                        ...
                        <EnquiryDialog dropdownData={dropdownData} state={state} locale={locale}>
                          <Button
                            variant={"black"}
                            className="min-w-[100px] sm:min-w-[120px] xl:min-w-[135px] 2xl:min-w-40 cursor-pointer"
                          >
        ...
        {data && data?.length > 1 && (
          <div>
            ...
            >
              <Image src="/images/icon-embla-prev.svg" alt="arrow prev" width={16} height={32} quality={90} />
            </PrevButton>
            <NextButton
            ...
            >
              <Image src="/images/icon-embla-next.svg" alt="arrow next" width={16} height={32} quality={90} />
            </NextButton>
          </div>
        )}
      </div>
