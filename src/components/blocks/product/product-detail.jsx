...
export default function ProductDetail({
  ...
  const enhanced = getLocalizedContent(
    isEn,
    initialData?.enhance,
    initialData?.enhance_ar,
  );
  ...
  const design_title = getLocalizedContent(
    isEn,
    initialData?.design_title,
    initialData?.design_title_ar,
  );
  ...
  const productImages =
    initialData?.images?.length > 0
      ...
      : Array(5).fill({
          media_type: "image",
          media_path: "/images/placeholder.png",
          alt: "placeholder",
        });
  ...
  const [toggleWishlist, { isLoading: isWishlistLoading }] =
    useToggleWishlistMutation();
  const { isAuthenticated } = useAppSelector((state) => state.auth);

  const cartItems = useSelector(selectCartItems) || [];
  const cartItem = cartItems.find(
    (cItem) => cItem.variant_id === initialData?.id,
  );
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      ...
      toast.success(
        wishlist
          ? `${tToast("wishlist_removed")}`
          : `${tToast("wishlist_success")}`,
      );
    } catch {
      setWishlist((prev) => !prev);
                ...
                >
                  ...
                  >
                    <>
                      ...
                      <div
                        className={cn(
                          "flex gap-4 xl:gap-3 2xl:gap-4 absolute z-2 top-3 xl:top-4",
                          locale === "ar"
                            ? "left-3 xl:left-4 2xl:left-5"
                            : "right-3 xl:right-4 2xl:right-5",
                        )}
                      >
                        <button
                          onClick={handleWishlistToggle}
                          disabled={isWishlistLoading}
                          className="w-5 xl:w-3 2xl:w-4.5 hover:cursor-pointer transition hover:scale-105 disabled:opacity-60 disabled:scale-100 flex"
                        >
                          <svg
                            width="15"
                            height="15"
                        ...
                        <button
                          ...
                          className="w-4 xl:w-3 2xl:w-3.5 hover:cursor-pointer transition hover:scale-105"
                        >
                          <Image
                ...
                <div className="w-full xl:max-w-[390px] 2xl:max-w-[468px] 3xl:max-w-[576px]">
                  ...
                  <div className="mb-2">
                    <div className="w-full max-w-[468px] bg-[#f2f2f2] border border-[#dedede] flex items-center p-1 rounded-[4px]">
                      ...
                      <div className="flex-1 flex justify-between gap-2 p-2 xl:p-2.5 2xl:p-[15px] ">
                        <div className="text-[12px] sm:text-[10px] 2xl:text-[12px] 3xl:text-[14px] leading-normal font-light text-[#282828]">
                          {isEn
                            ? initialData?.model_title
                            : initialData?.model_title_ar}
                        </div>
                        <ProductChooseDesign
                          data={initialData}
                          locale={locale}
                        ...
                        >
                          <Button
                            variant={"link"}
                            className={
                              "text-[10px] sm:text-[8px] xl:text-[10px] 2xl:text-[10px] 3xl:text-[12px] leading-normal font-light text-[#282828] h-auto! "
                            }
                          >
                  ...
                  {design_title && (
                    <Heading
                      as="div"
                      size="none"
                      className="text-[12px] sm:text-[10px] 2xl:text-[12px] 3xl:text-[14px] leading-normal font-light text-[#808080] mb-2 xl:mb-3 2xl:mb-5"
                    >
                      {design_title}
                    </Heading>
                  )}
                  ...
                  >
                    {t("common.aed")} {initialData?.price}{" "}
                    <span>{t("common.inc_tax")}</span>
                  </Heading>
                  {initialData?.stock > 0 && (
                    <PriceAndCart
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-5 2xl:gap-6 mt-4 xl:mt-0 mb-2 xl:mb-3 2xl:mb-5 max-lg:flex-wrap-reverse">
                    {enhanced && (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-5 xl:gap-4 2xl:gap-6">
                      <Button
                        variant={"link"}
                        className={"font-normal underline h-auto "}
                        disabled={initialData?.stock == 0}
                  ...
                  {boughtTogetherItems && (
                    <>
                      ...
                      <Heading
                        as="div"
                        size="heading5"
                        className="font-normal text-[#282828] mb-3 2xl:mb-4 mt-2.5 2xl:mt-4"
                      >
                        {t("product.frequently_bought_together")}
                      </Heading>
                      <FrequentBroughtTogether
                        data={boughtTogetherItems}
                        frequentlyEmblaRef={frequentlyEmblaRef}
                        locale={locale}
                      />
