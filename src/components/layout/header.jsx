...
import { Locale, locales, localeNames } from "@/il8n/config";
    ...
    <AnimatePresence mode="wait">
      <motion.header
        ...
        className={cn(
          ...
          bg ? "border-b border-white/10 bg-black/95 shadow-[0px_10px_4px_0px_rgba(0,0,0,0.1)] backdrop-blur-sm fixed" : "absolute"
        )}
      >
        <div className="container">
          <div className="flex justify-between items-center gap-x-3 lg:gap-x-8">
            <MediaQuery maxWidth={1023}>
              <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
                <SheetTrigger className="flex items-center gap-x-1.5 2xs:gap-x-2">
                  <Menu size={18} strokeWidth={1} className="size-5 text-white" />
                ...
                <SheetContent className="max-w-[320px] sm:max-w-[320px] bg-white" side={locale === "ar" ? "right" : "left"}>
                  ...
                  <Link href={"/"} className="text-[14px] leading-none font-light text-white h-(--header-y) bg-black flex items-center gap-x-2 px-4">
                    ...
                    <AnimatePresence mode="wait">
                      {sheetOpen && (
                        <motion.div key="menu-anim" variants={containerVariants} initial="hidden" animate="show" exit="exit">
                          <NavigationMenuBar pathname={pathname} menuItems={navigationData} onNavigationClick={handleNavigationLinkClick} />
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
            ...
            >
              ...
              <Button variant="none" size="none" onClick={() => setCommand(!command)} className="">
                <Image src="/images/icon-search.svg" alt="search" width={12} height={12} unoptimized className="w-[15px] 2xl:w-[18px]" />
              </Button>
              <Button variant="none" size="none" className="">
                <Image src="/images/icon-bag.svg" alt="bag" width={12} height={12} unoptimized className="w-[15px] 2xl:w-[18px]" />
              </Button>
              <Button variant="none" size="none" className="">
                <Image src="/images/icon-user.svg" alt="user" width={12} height={12} unoptimized className="w-[15px] 2xl:w-[18px]" />
              </Button>
              {locale == "ar" ? (
                <Button
                  variant="link"
...
function NavigationMenuBar({ pathname, menuItems, onNavigationClick }) {
  const getNavigationMenuTriggerStyle = (isActive) => {
    ...
    return `${baseStyle} ${isActive ? "text-primary border-transparent" : "text-black lg:text-white"}`;
  };
  return (
    <NavigationMenu viewport={false} className="w-full max-w-full justify-start lg:justify-center max-lg:[&>div]:w-full">
        ...
        {menuItems.map((item, i) => {
          ...
          return (
            <motion.div key={i} variants={itemVariants}>
              <NavigationMenuItem>
                <NavigationMenuLink asChild className={cn(getNavigationMenuTriggerStyle(isActive))}>
...
function SearchCammand({ command, setCommand, locale }) {
  const placeholders = ["Search by Category", "Most trending products", "Comfort, Funtionality & Style"];
    ...
    >
      <div className="container">
        <div className="w-full max-w-full mx-auto flex justify-between items-center gap-x-4 px-1 py-3">
          <Button variant="none" size="none" onClick={() => setCommand(false)} className="">
