    ...
    <section className="w-full pt-[calc(var(--header-y)_+_20px)] sm:pt-[calc(var(--header-y)_+_10px)] relative">
      <div className="container mb-3 xl:mb-5 2xl:mb-8">
        <Breadcrumb className="mb-1 xl:mb-2">
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink href={`/${locale}`}>{isEn ? "Home" : "الرئيسية"}</BreadcrumbLink>
            </BreadcrumbItem>
            {slug && (
              <BreadcrumbItem>
                {link ? (
      ...
      <div className="w-full aspect-992/640 sm:aspect-1920/740 overflow-hidden flex items-center relative z-0">
        ...
        <div className="container">
          <div className="w-full xl:max-w-1/2 py-[40px] xl:py-[60px] 2xl:py-[80px]">
            ...
            {(isEn ? data?.heroDescription : data?.heroDescription_ar) && (
              <Text as="div" size="text1" className="line-clamp-4 font-light text-white max-w-[72%]">
            ...
            {label && (
              <Button
                asChild
                variant="white"
                className="text-[12px] 2xl:text-[16px] min-w-[100px] sm:min-w-[120px] xl:min-w-[135px] 2xl:min-w-[200px] mt-[20px] xl:mt-[30px] 2xl:mt-[45px] px-2"
              >
                <Link href={getExternalLink(ctaHref, locale)} target={getTarget(ctaHref)}>
                  {label}
                </Link>
              </Button>
            )}
