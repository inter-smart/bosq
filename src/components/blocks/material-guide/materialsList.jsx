...
export default function MaterialsList({ data, locale, extraMaterialsInfo}) {
    const isEn = locale === "en";
    const materialCount = data?.items?.length || 0;
                        ...
                        <div className="container">
                            ...
                            <div className="w-full h-auto block">
                                {item?.info_list?.map((item, index) => (
                                    ...
                                    >
                                        <div
                                            className={cn("w-[var(--width)] md:w-[calc(100%-var(--width))] max-sm:mb-[15px] max-md:mb-[25px]",
                                                locale === "ar"
                                                    ? index % 2 === 0 ? "pl-[var(--gap)]" : "pr-[var(--gap)]"
                                                    : index % 2 === 0 ? "pr-[var(--gap)]" : "pl-[var(--gap)]"
                                            )}>
                                            {item?.title && (
                                                <Heading
                                                    as="h2"
                                        ...
                                        <div className="w-[var(--width)]">
                                            <div className="w-full h-full min-h-[190px] max-h-[720px] overflow-hidden block relative z-0">
                                                <Image
                                                    src={item?.media?.path}
                                                    alt={isEn? item?.media?.alt: item?.media_ar?.alt || "Image"}
                                                    width={760}
                                                    height={630}
                                                    className="w-full h-full object-cover group-hover:scale-110 transition duration-500"
                                                />
                                                {item?.icon && (
                                                    <div className="w-[40px] sm:w-[50px] 2xl:w-[60px] 3xl:w-[80px] h-auto aspect-square m-[10px] sm:m-[15px] 3xl:m-[20px] overflow-hidden absolute z-1 inset-[0_auto_auto_0] flex items-center justify-center">
                                                        <Image
                                                            src={item?.icon?.path}
                    ...
                    >
                        <div className="container">
                            <div className="w-full h-auto block">
                                    <div
                                        key={index}
                                        className={`group [--width:100%] md:[--width:380px] lg:[--width:510px] 2xl:[--width:610px] 3xl:[--width:760px] md:[--gap:40px] lg:[--gap:50px] xl:[--gap:80px] 2xl:[--gap:100px] 3xl:[--gap:125px] w-full h-auto mb-[30px] sm:mb-[50px] md:mb-[60px] lg:mb-[80px] xl:mb-[100px] 2xl:mb-[125px] 3xl:mb-[160px] last:mb-0 flex flex-wrap ${(materialCount + index) % 2 === 0 ? "flex-row" : "flex-row-reverse"}`}
                                    >
                                        <div
                                            className={cn("w-[var(--width)] md:w-[calc(100%-var(--width))] max-sm:mb-[15px] max-md:mb-[25px]",
                                                locale === "ar"
                                                    ? (materialCount + index) % 2 === 0 ? "pl-[var(--gap)]" : "pr-[var(--gap)]"
                                                    : (materialCount + index) % 2 === 0 ? "pr-[var(--gap)]" : "pl-[var(--gap)]"
                                            )}>
                                            {item?.title && (
                                                <Heading
                                                    as="h2"
