...
export default function ProductCard({
  ...
  const productSlug =
    product?.base_slug ||
    (product?.slug ? product.slug.replace(/^\/products\//, "") : "");
  ...
  const [prevWishlistKey, setPrevWishlistKey] = useState({
    id: product?.id,
    val: product?.isWishlisted,
  });
  if (
    prevWishlistKey.id !== product?.id ||
    prevWishlistKey.val !== product?.isWishlisted
  ) {
        ...
        <div className="group w-full block">
            ...
            {(product?.hover_media_path || product?.hoverMedia?.path) && (
              <Image
                ...
                alt={
                  isEn
                    ? product?.title || product?.name
                    : product?.title_ar || product?.name_ar || product?.name
                }
                width={550}
                height={440}
                quality={100}
          ...
          <div>
            <Heading
              as="h2"
              size="none"
              className="text-[10px] xl:text-[9px] 2xl:text-[10px] 3xl:text-[12px] leading-normal font-light truncate text-[#bbbcbc] mb-0.5"
            >
                  ...
                  product?.categories || [
                    {
                      ...
                      name_ar: product?.category_ar || product?.category,
                    },
                  ]
                )
            ...
            <Heading
              as="div"
              size="none"
              className="text-[12px] xl:text-[12px] 2xl:text-[13px] 3xl:text-[14px] leading-normal font-normal truncate text-[#282828] mb-0.5"
            >
              <Link href={productUrl}>
                {isEn
                  ? product?.title || product?.name
            ...
            <div className="flex items-center gap-0.5 xl:gap-1">
              ...
              (product?.colorVariant && product.colorVariant.length > 0)) ? (
                <>
                  {(product?.colorVariant || colorVariant)
                    ?.slice(0, 3)
                    .map((color, index) => (
                      <Link
                        key={"color" + index}
